...
from truenas_tui.tui.forms import (
    Form, FormField, BoolField, ChoiceField, IntField, SectionField, ListField,
)
...
def _iface_summary(iface: dict) -> str:
    ...
    configured = [_alias_str(a) for a in aliases
                  if a.get('type') in ('INET', 'INET6')]
    active     = [_alias_str(a) for a in state_aliases
                  if a.get('type') in ('INET', 'INET6')]
...
def _parse_alias_entry(entry: str) -> tuple[dict | None, str | None]:
    ...
    """
    if '/' not in entry:
        return None, TRANSLATE('Alias must be in CIDR notation (e.g. 192.168.1.1/24): {e}').format(e=entry)
    ...
    if not (0 <= plen <= max_prefix):
        return None, TRANSLATE('Prefix length out of range in alias: {e}').format(e=entry)
...
def _parse_ip_only(s: str) -> dict:
    ...
    return {'type': 'INET6' if isinstance(ip, ipaddress.IPv6Address) else 'INET',
            'address': s}
...
class NetworkInterfacePlugin(BasePlugin):
    ...
    def _get_status(self, session) -> str:
        try:
            ...
            if waiting is not None:
                return TRANSLATE('Changes applied. Press [p] to persist or they roll back in {n}s.').format(n=int(waiting))
    ...
    def _draw_list(self, stdscr, ifaces: list, selected: int, status: str) -> None:
        ...
        stdscr.addstr(0, 0, f' {title} '.center(sw),
                      pair(colors.HEADER) | curses.A_BOLD)

        if status:
            try:
                stdscr.addstr(1, 2, status[:sw - 3],
        ...
        for i, iface in enumerate(ifaces):
            ...
            attr = pair(colors.MENU_SELECTED) | curses.A_BOLD if i == selected else curses.A_NORMAL
            try:
                stdscr.addstr(row, 2, f'{summary:<{sw - 4}}', attr)
            except curses.error:
                pass

        hint = TRANSLATE('[↑↓] Navigate  [Enter] Edit  [n] New  [d] Delete  [a] Apply  [p] Persist  [q] Back')
        try:
    ...
    def _apply_changes(self, stdscr, session) -> None:
        try:
            ...
            message_dialog(stdscr, TRANSLATE('Applied'),
                           TRANSLATE('Changes applied. Network will roll back in ~60 seconds\n'
    ...
    def _persist_changes(self, stdscr, session) -> None:
        try:
            ...
            message_dialog(stdscr, TRANSLATE('Persisted'), TRANSLATE('Network changes persisted successfully.'))
        except Exception as e:
            message_dialog(stdscr, TRANSLATE('Error'), format_error(e))
    ...
    def _delete_interface(self, stdscr, session, iface: dict) -> None:
        ...
        if not confirm_dialog(stdscr, TRANSLATE('Confirm Delete'),
                              TRANSLATE('Delete interface {name}?').format(name=iface_name)):
            return
        try:
            session.call(Method.INTERFACE_DELETE, iface_id)
            message_dialog(stdscr, TRANSLATE('Deleted'),
                           TRANSLATE('Interface {name} deleted.\n'
                             'Use [a] from the list to apply changes.').format(name=iface_name))
        except Exception as e:
            message_dialog(stdscr, TRANSLATE('Error'), format_error(e))
    ...
    def _edit_interface(self, stdscr, session, iface: dict) -> None:
        ...
        try:
            ...
            message_dialog(stdscr, TRANSLATE('Saved'),
                           TRANSLATE('Interface {name} updated.\n'
                             'Use [a] from the list to apply changes.').format(name=iface_name))
        except Exception as e:
            message_dialog(stdscr, TRANSLATE('Error'), format_error(e))
    ...
    def _create_interface(self, stdscr, session) -> None:
        ...
        step1_fields = [
            ChoiceField('type', TRANSLATE('Interface Type'), choices=_IFACE_TYPES, value=0),
        ...
        if not iface_name:
            message_dialog(stdscr, TRANSLATE('Error'), TRANSLATE('Interface name is required.'))
            return
        ...
        fields = _build_edit_fields(stdscr, session, stub_iface, failover_licensed,
                                    name_readonly=False)
        if fields is None:
            return
        ...
        payload = _collect_payload(result, stub_iface, failover_licensed, is_create=True)
        if isinstance(payload, str):
            message_dialog(stdscr, TRANSLATE('Error'), payload)
            return
        ...
        try:
            ...
            message_dialog(stdscr, TRANSLATE('Created'),
                           TRANSLATE('Interface {name} created.\n'
                             'Use [a] from the list to apply changes.').format(name=iface_name))
        except Exception as e:
            message_dialog(stdscr, TRANSLATE('Error'), format_error(e))
...
# ---------------------------------------------------------------------------
# Field builder (shared between edit and create step-2)
# ---------------------------------------------------------------------------

def _build_edit_fields(stdscr, session, iface: dict, failover_licensed: bool,
                       name_readonly: bool = True) -> list | None:
    """
    ...
    fields.append(FormField('name', TRANSLATE('Name'), iface.get('name', ''),
                            readonly=name_readonly))
    fields.append(FormField('description', TRANSLATE('Description'),
                            iface.get('description', '')))

    # DHCP / IPv6 auto: only when NOT HA-licensed (mirrors midcli)
    if not failover_licensed:
        fields.append(BoolField('ipv4_dhcp', TRANSLATE('IPv4 DHCP'),
                                value=iface.get('ipv4_dhcp', False)))
        fields.append(BoolField('ipv6_auto', TRANSLATE('IPv6 Auto'),
                                value=iface.get('ipv6_auto', False)))

    fields.append(ListField(
        '_aliases', TRANSLATE('Aliases (CIDR)'),
        value=[_alias_str(a) for a in iface.get('aliases', [])
               if a.get('type') in ('INET', 'INET6')],
        item_label=TRANSLATE('Address'),
        item_validator=_validate_alias_str,
    ))

    # --- Failover section (HA only) ---
    if failover_licensed:
        fields.append(SectionField('', TRANSLATE('Failover Settings')))
        fields.append(BoolField('failover_critical', TRANSLATE('Failover Critical'),
                                value=iface.get('failover_critical', False)))
        fields.append(IntField('failover_group', TRANSLATE('Failover Group'),
                               value=iface.get('failover_group') or 1,
                               min_val=1))
        fields.append(ListField(
            '_failover_aliases', TRANSLATE('This Node IPs'),
            value=[a.get('address', '') for a in iface.get('failover_aliases', [])],
            item_label=TRANSLATE('IP Address'),
            item_validator=_validate_ip_only,
        ))
        fields.append(ListField(
            '_failover_virtual_aliases', TRANSLATE('Virtual IPs'),
            value=[a.get('address', '') for a in iface.get('failover_virtual_aliases', [])],
            item_label=TRANSLATE('IP Address'),
            item_validator=_validate_ip_only,
        ))
    ...
    if iface_type == 'VLAN':
        try:
            ...
            choices = sorted(choices_dict.keys()) if isinstance(choices_dict, dict) else sorted(choices_dict)
        except Exception as e:
        ...
        fields.append(ChoiceField('vlan_parent_interface', TRANSLATE('Parent Interface'),
                                  choices=choices,
                                  value=_choice_idx(choices,
                                                    iface.get('vlan_parent_interface', ''))))
        fields.append(IntField('vlan_tag', TRANSLATE('VLAN Tag'),
                               value=iface.get('vlan_tag') or 1, min_val=1, max_val=4094))
        fields.append(IntField('vlan_pcp', TRANSLATE('Priority (PCP)'),
                               value=iface.get('vlan_pcp') or 0, min_val=0, max_val=7))

    elif iface_type == 'BRIDGE':
        try:
            choices_dict = session.call(Method.INTERFACE_BRIDGE_MEMBERS_CHOICES,
                                        iface.get('name', ''))
            choices = sorted(choices_dict.keys()) if isinstance(choices_dict, dict) else sorted(choices_dict)
        except Exception as e:
            message_dialog(stdscr, TRANSLATE('Error'),
        ...
        fields.append(ListField(
            'bridge_members', TRANSLATE('Members'),
        ...
        try:
            port_dict = session.call(Method.INTERFACE_LAG_PORTS_CHOICES,
                                     iface.get('name', ''))
            port_choices = sorted(port_dict.keys()) if isinstance(port_dict, dict) else sorted(port_dict)
        except Exception as e:
            message_dialog(stdscr, TRANSLATE('Error'),
                           TRANSLATE('Could not load LAG port choices:\n') + format_error(e))
            return None
        fields.append(SectionField('', TRANSLATE('Link Aggregation Settings')))
        fields.append(ChoiceField('lag_protocol', TRANSLATE('Protocol'),
                                  choices=_LAG_PROTOCOLS,
                                  value=_choice_idx(_LAG_PROTOCOLS,
                                                    iface.get('lag_protocol', 'LACP'))))
        fields.append(ListField(
            'lag_ports', TRANSLATE('Ports'),
        ...
        fields.append(ChoiceField('xmit_hash_policy', TRANSLATE('Xmit Hash Policy'),
                                  choices=_XMIT_POLICIES,
                                  value=_choice_idx(_XMIT_POLICIES,
                                                    iface.get('xmit_hash_policy', 'LAYER2+3'))))
        fields.append(ChoiceField('lacpdu_rate', TRANSLATE('LACPDU Rate'),
                                  choices=_LACPDU_RATES,
                                  value=_choice_idx(_LACPDU_RATES,
                                                    iface.get('lacpdu_rate', 'SLOW'))))

    # --- Other Settings ---
    fields.append(SectionField('', TRANSLATE('Other Settings')))
    fields.append(IntField('mtu', TRANSLATE('MTU'),
                           value=iface.get('mtu') or 0, min_val=0, max_val=9000,
...
# ---------------------------------------------------------------------------
# Payload builder (shared between edit and create)
# ---------------------------------------------------------------------------

def _collect_payload(result: dict, iface: dict, failover_licensed: bool,
                     is_create: bool) -> dict | str:
    """
