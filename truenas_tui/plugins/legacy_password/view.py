...
from truenas_tui.tui.dialogs import message_dialog, select_dialog, input_dialog
...
class PasswordPlugin(BasePlugin):
    ...
    def _fetch_label(self, session) -> str:
        try:
            ...
            has_admin = True
        return (
            TRANSLATE('Change local administrator password') if has_admin
    ...
    def _change_password(self, stdscr, session) -> None:
        ...
        if not admins:
            message_dialog(stdscr, TRANSLATE('Error'),
                           TRANSLATE('No local administrators found.'))
            return
        ...
        if len(pw1) < _MIN_PASSWORD_LEN:
            message_dialog(stdscr, TRANSLATE('Error'),
                           TRANSLATE('Password must be at least {n} characters.').format(n=_MIN_PASSWORD_LEN))
            return

        pw2 = input_dialog(stdscr,
                           TRANSLATE('Change Password'),
        ...
        try:
            ...
            message_dialog(stdscr, TRANSLATE('Success'),
                           TRANSLATE('Password for {u} changed successfully.\n'
                             'Two-factor authentication has been disabled.').format(u=username))
        except Exception as e:
            message_dialog(stdscr, TRANSLATE('Error'), format_error(e))
    ...
    def _setup_administrator(self, stdscr, session) -> None:
        ...
        try:
            ...
            message_dialog(stdscr, TRANSLATE('Success'),
                           TRANSLATE('Local administrator {u} set up successfully.').format(u=username))
        except Exception as e:
            message_dialog(stdscr, TRANSLATE('Error'), format_error(e))
