...
class ResetConfigPlugin(BasePlugin):
    ...
    def run(self, stdscr, session) -> None:
        ...
        if not confirm_dialog(
            ...
            TRANSLATE('This will ERASE all configuration and reset to defaults.\n'
        ...
        if typed != 'RESET':
            message_dialog(stdscr, TRANSLATE('Cancelled'),
                           TRANSLATE('Configuration reset cancelled.'))
            return

        try:
            session.call(Method.SYSTEM_CONFIG_RESET)
            message_dialog(stdscr, TRANSLATE('Reset'),
                           TRANSLATE('Configuration reset initiated.\n'
