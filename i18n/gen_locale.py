...
def generate_locale(
    i18n_dir: pathlib.Path = I18N_DIR, locale_dir: pathlib.Path = LOCALE_DIR
) -> None:
        ...
        for domain in domains:
            ...
            if not msgfmt_missing:
                ...
                if result.returncode != 0:
                    print(
                        f"WARNING: msgfmt failed: {result.stderr.decode().strip()}",
                        file=sys.stderr,
                    )
                    msgfmt_missing = True
...
def update_json(
    i18n_dir: pathlib.Path = I18N_DIR,
    ...
    for json_file in json_files:
        ...
        missing = [
            s for s in sorted(all_strings) if not _get_translation(data.get(s, {}))
        ]
        ...
        for attempt in range(2):
            print(
                f"  {lang} ({gt_code}): translating {len(missing)} strings"
        ...
        print(
            f"  {lang}: done — {inserted} inserted, {failed} failed{stale_msg}",
            flush=True,
        )
