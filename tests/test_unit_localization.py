...
# ---------------------------------------------------------------------------
# Fallback for unknown / invalid languages
# ---------------------------------------------------------------------------
...
def test_fallback_english_invalid_lang_rejected():
    ...
    for bad in ('../../etc', '', '  ', 'en; rm -rf /', '../passwd'):
        ...
        assert loc._language == 'en', f"Expected 'en' for bad lang {bad!r}, got {loc._language!r}"
    loc.setup_locale('en')  # reset
...
# ---------------------------------------------------------------------------
# Completeness: every TRANSLATE() string must be a key in every i18n JSON
# ---------------------------------------------------------------------------
    ...
    for json_path in sorted(I18N_DIR.glob('*.json')):
        ...
        for key, val in data.items():
            if not isinstance(val, dict):
                shape_report.append(f"{json_path.name}: {key!r} value must be a dict, got {type(val).__name__!r}")
            elif "translation" not in val or "machine_translated" not in val:
                shape_report.append(f"{json_path.name}: {key!r} missing 'translation' or 'machine_translated'")
