...
def test_collect_failover_licensed():
    ...
    payload = _collect_payload(result, {'type': 'PHYSICAL'}, failover_licensed=True, is_create=False)
    assert isinstance(payload, dict)
    ...
    assert payload['failover_virtual_aliases'] == [{'type': 'INET', 'address': '10.0.0.254'}]
...
# ---------------------------------------------------------------------------
# NetworkInterfacePlugin._get_status()
# ---------------------------------------------------------------------------
...
def test_get_status_waiting():
    ...
    assert '45' in status
    assert 'persist' in status.lower() or 'roll' in status.lower() or 'p]' in status.lower()
...
# ---------------------------------------------------------------------------
# NetworkInterfacePlugin._apply_changes()
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# NetworkInterfacePlugin._persist_changes()
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# NetworkInterfacePlugin._delete_interface()
# ---------------------------------------------------------------------------
...
def test_delete_confirmed():
    ...
    with patch('truenas_tui.plugins.network_interface.view.confirm_dialog', return_value=True), \
...
def test_delete_cancelled():
    ...
    with patch('truenas_tui.plugins.network_interface.view.confirm_dialog', return_value=False):
        plugin._delete_interface(stdscr, session, iface)
