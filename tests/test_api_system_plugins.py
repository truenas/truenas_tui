...
class TestRebootPlugin:
    ...
    def test_confirmed_calls_system_reboot(self, recording_session, stdscr):
        with patch('truenas_tui.plugins.legacy_reboot.view.input_dialog',
                   return_value='Maintenance window') as _, \
             patch('truenas_tui.plugins.legacy_reboot.view.confirm_dialog',
                   return_value=True), \
    ...
    def test_cancel_at_reason_dialog_no_api_call(self, recording_session, stdscr):
        with patch('truenas_tui.plugins.legacy_reboot.view.input_dialog', return_value=None):
            RebootPlugin().run(stdscr, recording_session)
    ...
    def test_blank_reason_treated_as_cancel(self, recording_session, stdscr):
        with patch('truenas_tui.plugins.legacy_reboot.view.input_dialog', return_value='   '):
            RebootPlugin().run(stdscr, recording_session)
    ...
    def test_cancel_at_confirm_dialog_no_api_call(self, recording_session, stdscr):
        with patch('truenas_tui.plugins.legacy_reboot.view.input_dialog', return_value='reason'), \
             patch('truenas_tui.plugins.legacy_reboot.view.confirm_dialog', return_value=False):
            RebootPlugin().run(stdscr, recording_session)
...
class TestShutdownPlugin:
    ...
    def test_confirmed_calls_system_shutdown(self, recording_session, stdscr):
        with patch('truenas_tui.plugins.legacy_shutdown.view.input_dialog',
                   return_value='Planned maintenance'), \
             patch('truenas_tui.plugins.legacy_shutdown.view.confirm_dialog',
                   return_value=True), \
    ...
    def test_cancel_at_reason_no_api_call(self, recording_session, stdscr):
        with patch('truenas_tui.plugins.legacy_shutdown.view.input_dialog', return_value=None):
            ShutdownPlugin().run(stdscr, recording_session)

        assert not recording_session.was_called(Method.SYSTEM_SHUTDOWN)

    def test_blank_reason_treated_as_cancel(self, recording_session, stdscr):
        with patch('truenas_tui.plugins.legacy_shutdown.view.input_dialog', return_value=''):
            ShutdownPlugin().run(stdscr, recording_session)

        assert not recording_session.was_called(Method.SYSTEM_SHUTDOWN)

    def test_cancel_at_confirm_no_api_call(self, recording_session, stdscr):
        with patch('truenas_tui.plugins.legacy_shutdown.view.input_dialog', return_value='reason'), \
             patch('truenas_tui.plugins.legacy_shutdown.view.confirm_dialog', return_value=False):
            ShutdownPlugin().run(stdscr, recording_session)
...
class TestResetConfigPlugin:
    ...
    def test_correct_string_calls_system_config_reset(self, recording_session, stdscr):
        with patch('truenas_tui.plugins.legacy_reset_config.view.confirm_dialog', return_value=True), \
             patch('truenas_tui.plugins.legacy_reset_config.view.input_dialog', return_value='RESET'), \
    ...
    def test_first_confirm_cancel_no_api_call(self, recording_session, stdscr):
        with patch('truenas_tui.plugins.legacy_reset_config.view.confirm_dialog', return_value=False):
            ResetConfigPlugin().run(stdscr, recording_session)
    ...
    def test_wrong_string_no_api_call(self, wrong, stdscr):
        ...
        with patch('truenas_tui.plugins.legacy_reset_config.view.confirm_dialog', return_value=True), \
             patch('truenas_tui.plugins.legacy_reset_config.view.input_dialog', return_value=wrong), \
             patch('truenas_tui.plugins.legacy_reset_config.view.message_dialog'):
    ...
    def test_none_input_no_api_call(self, recording_session, stdscr):
        with patch('truenas_tui.plugins.legacy_reset_config.view.confirm_dialog', return_value=True), \
             patch('truenas_tui.plugins.legacy_reset_config.view.input_dialog', return_value=None), \
             patch('truenas_tui.plugins.legacy_reset_config.view.message_dialog'):
            ResetConfigPlugin().run(stdscr, recording_session)
    ...
    def test_cancel_message_shown_for_wrong_string(self, recording_session, stdscr):
        with patch('truenas_tui.plugins.legacy_reset_config.view.confirm_dialog', return_value=True), \
             patch('truenas_tui.plugins.legacy_reset_config.view.input_dialog', return_value='wrong'), \
             patch('truenas_tui.plugins.legacy_reset_config.view.message_dialog') as mock_msg:
            ResetConfigPlugin().run(stdscr, recording_session)
