...
# ---------------------------------------------------------------------------
# _collect()  — called directly without running the event loop
# ---------------------------------------------------------------------------
...
def test_form_up_navigation():
    ...
    stdscr.getch.side_effect = [curses.KEY_DOWN, curses.KEY_UP, ord('\t'), ord('\t'), ord('\n')]
...
# ---------------------------------------------------------------------------
# Form._field_handle_key() — text entry in FormField / IntField
# ---------------------------------------------------------------------------
...
def test_text_field_backspace():
    ...
    fields = [FormField('name', 'Name', '')]
    form = Form(stdscr, 'Test', fields)
    stdscr.getch.side_effect = [ord('h'), ord('i'), curses.KEY_BACKSPACE, ord('\t'), ord('\n')]
...
def test_text_field_ctrl_u_clears():
    ...
    fields = [FormField('name', 'Name', '')]
    form = Form(stdscr, 'Test', fields)
    stdscr.getch.side_effect = [ord('h'), ord('i'), 21, ord('\t'), ord('\n')]  # Ctrl+U = 21
