...
def _make_session(username='admin', hostname='testnas', version='25.10.0',
                  role_label='FULL_ADMIN', server='192.168.1.108'):
    session = MagicMock()
...
# ---------------------------------------------------------------------------
# _fmt_bytes / _fmt_load (already tested above, pure)
# ---------------------------------------------------------------------------
...
def test_q_exits(stdscr):
    ...
    with patch('curses.curs_set'), patch('curses.color_pair', return_value=0), \
         patch('curses.doupdate'), patch('truenas_tui.tui.main_view.pair', return_value=0):
